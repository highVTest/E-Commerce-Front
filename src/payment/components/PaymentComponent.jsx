...
function PaymentComponent({token, paymentData, totalPrice}){
    const cartIdList = []
    const productIdList = []
    ...
    const getBuyerCoupon = async () => {
        ...
        const filterData = data.data.filter(coupon => productIdList.includes(coupon.productId))
            ...
            <Fieldset fw={1000} legend="사용 가능 쿠폰">
                {
                    coupons.map((coupon) => {
                        return(
                            <>
                                <Card shadow="sm" padding="xs" component="a" target="_blank" style={{display: "flex", justifyContent: "space-between"}}>
                                    <div className="field-set" style={{display: 'flex', alignItems: "center"}}>
                                        ...
                                        <div style={{width:"80%"}}>
                                            ...
                                            <Text fw={500} size="lg" mt="md">
                                                만료 시간 : {coupon.expiredAt.split('-')[0]} 년 {coupon.expiredAt.split('-')[1]} 월 {coupon.expiredAt.split('-')[2].slice(0,2)} 일 까지
                                            </Text>
                                            {
